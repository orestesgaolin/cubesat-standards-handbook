...
\input{book_data} % File with basic book data such as author, title, subtitle
...
\draw[anchor=north] (midpoint) node [fill=ocre!30!white,fill opacity=0.6,text opacity=1,inner sep=1cm]{\Huge\centering\bfseries\sffamily\parbox[c][][t]{\paperwidth}{\centering \bookTitle \\ [15pt] % Book title
{\Large \bookSubtitle } \\[2pt] % Subtitle
{\huge }}}; % Author name
\end{tikzpicture}};
\end{tikzpicture}
\vfill
...
\noindent \textsc{Edited by} \bookEditor \\ 

\noindent \textsc{Published by} \bookPublisher~\url{\bookURL} \\
